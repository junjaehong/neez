...
public class BizCardReaderController {
    ...
    // ✅ 1-1. OCR (서버에 있는 파일명으로 처리)
    @Operation(
            summary = "명함 OCR 등록(서버 파일명)",
            description = "서버에 이미 존재하는 명함 이미지 파일명을 받아 OCR 후 저장합니다."
    )
    @PostMapping("/read")
    ...
    // ✅ 1-2. 파일 업로드(OCR) - multipart/form-data 전용
    @Operation(
            summary = "명함 이미지 업로드 + OCR (multipart)",
            description = "multipart/form-data 로 이미지를 업로드하고 OCR 후 저장합니다."
    )
    @PostMapping(
            value = "/read/upload",
    ...
    // ✅ 1-3. 파일 업로드(OCR) - application/octet-stream 전용
    @Operation(
            summary = "명함 이미지 업로드 + OCR (octet-stream)",
            description = "바이너리(body)로 이미지를 전송하고 OCR 후 저장합니다. user_idx는 쿼리스트링으로 전달하세요."
    )
    @PostMapping(
            value = "/read/upload",
    ...
    public ResponseEntity<ApiResponseDto<BizCardDto>> uploadAndOcrBytes(
        ...
        try {
            String storedFileName = bizCardReaderService.storeBizCardImage(rawBytes, "upload.jpg");
    ...
    @Operation(
            summary = "명함 수기 등록",
            description = "수기로 입력한 명함 정보를 JSON으로 받아 저장합니다."
    )
    @PostMapping("/manual")
    ...
    // ✅ 4. 사용자 명함 목록 (페이징)
    @Operation(summary = "사용자 명함 목록 조회", description = "특정 사용자의 명함 목록을 페이징으로 조회합니다.")
    ...
    // ✅ 8. 삭제 (Soft Delete)
    @Operation(summary = "명함 삭제", description = "명함을 실제 삭제하지 않고 is_deleted=true로 표시합니다.")
    @DeleteMapping("/{idx}")
    ...
    // ✅ 10. 복구
    @Operation(summary = "명함 복구", description = "삭제(is_deleted=true)된 명함을 복구합니다.")
    ...
    // ✅ 12. 중복 확인
    @Operation(summary = "명함 중복 확인", description = "이름+이메일 기준으로 명함이 이미 존재하는지 확인합니다.")
    ...
    // ✅ 13. 삭제된 명함 목록
    @Operation(summary = "삭제된 명함 목록 조회", description = "소프트 삭제된 명함 목록을 페이징으로 조회합니다.")
