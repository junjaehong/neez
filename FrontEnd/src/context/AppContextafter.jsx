...
export const AppProvider = ({ children }) => {
  ...
  // 해시태그 목록
  const [hashtags, setHashtags] = useState(['중요', '거래처', '개발', '마케팅', '채용', '인사']);
  ...
  // 카드별 회의록 저장소 (cardId -> notes)
  const [meetingNotesStore, setMeetingNotesStore] = useState({});
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      if (parsedData.meetingNotesStore) setMeetingNotesStore(parsedData.meetingNotesStore);
    }
  }, []);
  ...
  }, [myCard, cardList, currentUser, settings, hashtags, meetingNotesStore]);
  ...
  const addMeetingNote = (cardId, note) => {
    ...
    // 카드 목록에도 반영
    setCardList(cardList.map(card => {
      const cardKey = String(card.id ?? card.idx);
      if (cardKey === targetId) {
        return { 
          ...card, 
          meetingNotes: [...(card.meetingNotes || []), newNote]
        };
      }
      return card;
    }));

    // 별도 회의록 저장소에도 저장 (API 카드 목록과 무관하게 표시 가능)
    setMeetingNotesStore(prev => {
      const existing = prev[targetId] || [];
      return {
        ...prev,
        [targetId]: [...existing, newNote]
      };
    });
  };
